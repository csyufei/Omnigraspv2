...
class HumanoidOmniGrasp(humanoid_amp_task.HumanoidAMPTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        temp_motion_file = 'sample_data/grab_sample.pkl'
        # '''
        data_seq = joblib.load(temp_motion_file)
        
        data_seq = {k: v for k, v in data_seq.items() if k == 'watch'}       ### maintain 'bowl'
        ...
        for data_key in list(data_seq.keys()):
            ...
            self.obj_name_to_code[obj_name] = joblib.load(f"sample_data/{obj_name}_bps.pkl")['bps_code']          ####### how to align???
        ...
        self.right_hand_body_ids = self._build_key_body_ids_tensor(cfg.env.right_hand_bodies)
    ...
    def _load_target_asset_ori(self): 
        # import pdb; pdb.set_trace()
        self._target_assets = {}
    ...
    def _build_target(self, env_id, env_ptr):
        ...
        ################## load obj keypoints while building the env ##################
        # 加载mesh并提取关键点
        mesh_type = "ply" if obj_name in ['Apple', 'Airplane', 'Alarmclock'] else "obj"
        mesh_path = os.path.join('phc/data/assets/urdf', obj_name, f'top_watertight_tiny.{mesh_type}')
    ...
    def _build_target_ori(self, env_id, env_ptr):
        default_pose = gymapi.Transform()
    ...
    def _reset_target(self, env_ids):
        n = len(env_ids)
        motion_times = torch.zeros(n, device=self.device).int()
        motion_ids = self._motion_lib2.sample_motions(n)
        hoi_data_batch, init_root_pos, init_root_rot,  init_root_pos_vel, init_root_rot_vel, init_hand_dof_pos, init_hand_dof_pos_vel, \
        self.init_obj_pos[env_ids], self.init_obj_pos_vel[env_ids], self.init_obj_rot[env_ids], self.init_obj_rot_vel[env_ids], \
        init_obj2_pos, init_obj2_pos_vel, init_obj2_rot, init_obj2_rot_vel \
            = self._motion_lib2.get_initial_state(env_ids, motion_ids, motion_times)
    ...
    def _compute_reward(self, actions):
        ...
        hand_pos = self._rigid_body_pos[:, self.right_hand_body_ids, :]
        hand_rot = self._rigid_body_rot[:, self.right_hand_body_ids, :]
        hand_vel = self._rigid_body_vel[:, self.right_hand_body_ids, :]
        hand_ang_vel = self._rigid_body_ang_vel[:, self.right_hand_body_ids, :]
    ...
    def pre_physics_step(self, actions):
        ...
        self._hand_pos_prev = self._rigid_body_pos[:, self.right_hand_body_ids, :].clone()
